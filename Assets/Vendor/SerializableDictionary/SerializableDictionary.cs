...
public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable
{
	...
	public bool FirstMatch(string pattern, out TValue result)
	{
		var mDict = (IDictionary<TKey, TValue>)m_dict;
		var firstOrDefault = mDict.Keys.FirstOrDefault(k => pattern.Contains(k.ToString()));
		if (firstOrDefault != null)
		{
			result = mDict[firstOrDefault];
			return true;
		}
		result = default;
		return false;
